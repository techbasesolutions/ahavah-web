/**
 * Icon-only Ahavah lockup — the rounded indigo square with the lime
 * heart-A glyph and the indigo teardrop knockout. Inline SVG so it
 * scales cleanly at any size and accepts a `color` prop for the glyph
 * fill (defaults to brand lime). This is the canonical mark; replaces
 * every prior 4-point-sparkle SVG (deleted per brand sign-off 2026-05-19).
 *
 * For the full lockup with wordmark, use <Logo variant="horizontal" /> or
 * <Logo variant="stacked" /> from logo.tsx.
 */

type Props = {
  size?: number;
  className?: string;
  /** Override the indigo tile background (e.g. transparent for overlays). */
  tileColor?: string;
  /** Override the lime heart-A fill. */
  glyphColor?: string;
  /** Override the indigo teardrop knockout color. */
  knockoutColor?: string;
  /** Tile corner radius in viewBox units (145 wide). Default 30. Raise it to
   *  approximate an OS rounded-app-icon mask (e.g. the PWA launch icon). */
  radius?: number;
  /** Hides from screen readers when used decoratively. */
  decorative?: boolean;
};
...
export function LogoMark({
  ...
  return (
    <svg
      ...
      {...(decorative
        ? { "aria-hidden": true }
        : { role: "img", "aria-label": "Ahavah" })}
    >
      <rect width="145" height="145" rx={radius} fill={tileColor} />
      <path
