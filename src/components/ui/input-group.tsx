...
// Surface tones mirror Input.tone:
//   default  — light-theme border-input + dark:bg-input/30 (shadcn default)
//   elevated — border-0 + bg-bg-elevated (Ahavah dark surface)
// Focus-visible and aria-invalid styling stays in the base because both
// tones want the same a11y treatment.
...
function InputGroup({
  ...
  return (
    <div
      ...
      role="group"
      className={cn(inputGroupVariants({ tone }), className)}
      {...props}
    />
  )
}
...
// `size` omitted — collides with our extended Input/Textarea string-union
// `size` variant (default | sm | lg). The native HTML `size` attribute is
// rarely used and inapplicable inside an InputGroup anyway.
